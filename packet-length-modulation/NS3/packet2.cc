...
std::ofstream decodedResultsStream("decoded_results_summary.txt", std::ios::out);
...
int main(int argc, char *argv[])
{
    ...
    // Read the CSV file
    std::vector<PacketInfo> packetInfos = ReadCsvFile("weir.csv"); 
    ...
    // Run the simulation
    Simulator::Stop(Seconds(600.0)); // Extended simulation time to 10 minutes
    Simulator::Run();
    Simulator::Destroy();
